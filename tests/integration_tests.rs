...
impl Job for EmailJob {
    ...
    async fn execute(&self) -> Result<Self::Output, Self::Error> {
        ...
        if self.should_fail {
            Err(format!("Failed to send email {} after {} attempts", self.id, attempts))
        } else {
            Ok(format!("Email {} sent to {} with subject: {}", self.id, self.to, self.subject))
        }
    }
}
...
async fn setup_redis_queue(queue_name: &str) -> RedisQueue<EmailJob> {
    ...
    RedisQueue::new(redis_url, queue_name)
...
async fn test_complete_workflow() {
    ...
    // Push jobs to queue
    queue.push(successful_job.clone()).await.expect("Failed to push successful job");
    queue.push(failing_job.clone()).await.expect("Failed to push failing job");
